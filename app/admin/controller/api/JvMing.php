...
//        $login_url = 'https://www.juming.com/user_zh/p_login';
        ...
        $token_data = $this->client->request('GET','http:/'.sysconfig('ip','localhost').':5001/get_token')->getBody()->getContents();
...
//            'cookies'=>$jar,
        ...
        if (strstr($result['msg'],'登陆成功')) {
            ...
            foreach ($this->client->getConfig('cookies') as $item){
                if ($item->getName() == 'PHPSESSID'){
                    ...
                    YmAccount::where('account','=',$this->username)
    ...
    //下载删除域名数据
    public function download_delete_ym($scsj=''){
        $url = 'http://7a08c112cda6a063.juming.com:9696/newcha/del_down?scsj='.$scsj;
        $headers = [
            'cookie'=> $this->cookie,
            'user-agent'=> 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Safari/537.36',
        ];
        try {
            $resp = $this->client->request('GET',$url,[
                'headers'=>$headers,
                'sink' => 'delete_ym_'.$scsj.'.txt'
            ])->getBody()->getContents();
            if (strstr($resp,'您还未登录帐户')){
                $this->cookie = $this->login();
                if ($this->cookie =='帐户或密码错误!'){
                    $data = ['code'=>999,'msg'=>'账号【'.$this->username.'】帐户或密码错误!'];
                    return $data;
                }
                return $this->download_delete_ym($scsj);
            }
            return true;
        }catch (\Exception $e){
            $data = ['code'=>999,'msg'=>'账号【'.$this->username.'】 错误：'.$e->getMessage()];
