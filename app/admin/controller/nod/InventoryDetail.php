...
/**
 * @ControllerAnnotation(title="报表 库存盘点")
 */
class InventoryDetail extends AdminController
{
    ...
    public function index()
    {
        if ($this->request->isAjax()){
            $all_warehouse = $this->warehouse_model->select()->toArray();
            $list = [];
            foreach ($all_warehouse as $item){
                $count = $this->model->where('warehouse_id','=',$item['id'])->count();
                $sum = $this->model->where('warehouse_id','=',$item['id'])->sum('unit_price');
                $info = $this->model->field('good_name')->where('warehouse_id','=',$item['id'])->select()->toArray();
                $list[] = [
                    'count'=>$count,
                    'price'=>$sum,
                    'info'=>$info,
