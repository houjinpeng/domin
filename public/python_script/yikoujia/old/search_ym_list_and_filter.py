...
class SearchYmAndFilter():
    def __init__(self,filter):
        ...
        self.headers = {
            ...
            'Host': 'x.juming.com',
            'Origin': 'http://x.juming.com:',
            'Pragma': 'no-cache',
            'Referer': 'http://x.juming.com/ykj/',
    ...
    def get_cookie(self):
        ...
        cur = conn.cursor()
        sql = 'select * from ym_yikoujia_account_pool where is_daili=1'
        cur.execute(sql)
        user_data = random.choice(list(cur.fetchall()))
        cur.close()
        conn.close()
        return user_data
    ...
    def beian_worker(self):
        ...
        while True:
            ...
            log.info(f'备案查询剩余任务：{task_queue.qsize()} 当前数据:{ym_data}')
    ...
    def baidu_worker(self):
        while True:
            if task_queue.empty():
                ...
                continue
            ym_data = task_queue.get()
            log.info(f'百度 查询剩余任务：{task_queue.qsize()} 当前数据:{ym_data}')
            if ym_data['bdsl'] > 0:
                # 有直接放入redis 没有过滤
                redis_cli.sadd(f'ym_data_{self.filter["id"]}', json.dumps(ym_data))
                # log.info(f'插入购买查询队列中 {ym_data}')
    ...
    def sogou_worker(self):
        while True:
            if task_queue.empty():
                ...
                continue
            ym_data = task_queue.get()
            log.info(f'搜狗 查询剩余任务：{task_queue.qsize()} 当前数据:{ym_data}')
            if ym_data['sgsl'] > 0:
                # 有备案直接放入redis 没有过滤
                redis_cli.sadd(f'ym_data_{self.filter["id"]}', json.dumps(ym_data))
                log.info(f'插入购买查询队列中 {ym_data}')
    ...
    def so_worker(self):
        while True:
            if task_queue.empty():
                ...
                continue
            ym_data = task_queue.get()
            log.info(f'360 查询剩余任务：{task_queue.qsize()} 当前数据:{ym_data}')
            if ym_data['sosl'] > 0:
                # 有备案直接放入redis 没有过滤
                redis_cli.sadd(f'ym_data_{self.filter["id"]}', json.dumps(ym_data))
                log.info(f'插入购买查询队列中 {ym_data}')
    ...
    def history_worker(self):
        while True:
            if task_queue.empty():
                ...
                continue
            ym_data = task_queue.get()
            log.info(f'历史 查询剩余任务：{task_queue.qsize()} 当前数据:{ym_data}')
            if ym_data['jzls'] > 0:
                redis_cli.sadd(f'ym_data_{self.filter["id"]}', json.dumps(ym_data))
                log.info(f'插入购买查询队列中 {ym_data}')
    ...
    def wu_work(self):
        while True:
            if task_queue.empty():
                ...
                continue
            ym_data = task_queue.get()
            log.info(f'查询剩余任务：{task_queue.qsize()} 当前数据:{ym_data}')
            redis_cli.sadd(f'ym_data_{self.filter["id"]}', json.dumps(ym_data))
            log.info(f'插入购买查询队列中 {ym_data}')
