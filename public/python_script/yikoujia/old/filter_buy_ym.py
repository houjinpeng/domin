    ...
    def comp_beian(self, domain, beian):
        ...
        try:
            ...
            if len(beian_info['params']['list']) == 0:
                domain['cause'] = '没有备案'
                log.debug(f' 域名为：{domain["ym"]} 备案 {domain["cause"]}')
                # out_ym_quque.put(domain)
                return domain['cause']
            ...
            if xingzhi in self.filter_dict['beian']['beian_xz'].split(','):
                ...
                log.debug(f'域名为：{domain["ym"]} 备案 域名性质为：{xingzhi}')
                # out_ym_quque.put(domain)
                return domain['cause']
    ...
    def check_mingan(self, history_data):
        try:
            if history_data['data'] == None:
                log.info(f'剩余任务：{work_queue.qsize()}   历史对比完毕 没有历史')
                return True
        except Exception as e:
            return '没有历史'
    ...
    def work(self, beian=None, baidu=None,sogou=None,so=None):
        ...
        while True:
            ...
            if self.filter_data['place_1'] > int(domain_data['jg']) or int(domain_data['jg']) > self.filter_data['place_2'] :
                log.info(f'购买金额不付 域名价格{domain_data["jg"]}')
                self.save_out_data(domain_data)
                continue

            log.info(f'剩余任务:{work_queue.qsize()}  域名开始对比：{domain_data["ym"]}')

            # 对比历史
            if self.filter_dict.get('history'):
                is_ok = self.get_history_comp(domain_data)  # 返回失败信息
                if is_ok != True:
                    log.info({'ym': domain_data['ym'], 'id': domain_data['id'], 'cause': is_ok})
                    self.save_out_data(domain_data)
                    continue
            ...
            if self.filter_dict.get('zcs'):
                if self.ckeck_zhuceshang(domain_data) != True:
                    self.save_out_data(domain_data)
                    log.info({'ym': domain_data['ym'], 'id': domain_data['id'], 'cause': '注册商包含非法字符串'})
                    continue
            ...
            log.info({'ym': domain_data['ym'], 'id': domain_data['id'], 'cause': '需要购买'})
            self.save_out_data(domain_data)
