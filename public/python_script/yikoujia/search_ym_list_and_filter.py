...
class SearchYmAndFilter():
    ...
    def parse_info(self,resp):
        ...
        for data in new_data:
            self.log_queue.put(f"{str(datetime.datetime.now())[:19]}  插入域名：{data['ym']}-{data['jg']}")
        ...
        while True:
            ...
            if onece:
                ...
                for i in range(1, 1000000):
                    self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 开始查询列表第{i}页')
                    self.data['page'] = i
                    ...
                    conn.commit()
                    # self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 查询总数：{info["count"]}')
    ...
    def baidu_worker(self):
        ...
        while True:
            ...
            if int(info['sl']) > 0:
                ...
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 百度 查询剩余任务：{self.task_queue.qsize()}  插入购买查询队列中 {ym_data["ym"]}')
            else:
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 百度 查询剩余任务：{self.task_queue.qsize()} 过滤当前数据:{ym_data["ym"]}')
    ...
    def sogou_worker(self):
        ...
        while True:
            ...
            if info['sl'] > 0:
                ...
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 搜狗 查询剩余任务：{self.task_queue.qsize()}  插入购买查询队列中 {ym_data["ym"]}')
            else:
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 搜狗 查询剩余任务：{self.task_queue.qsize()} 过滤当前数据:{ym_data["ym"]}')
    ...
    def so_worker(self):
        ...
        while True:
            ...
            if info['sl'] > 0:
                ...
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 360 查询剩余任务：{self.task_queue.qsize()}  插入购买查询队列中 {ym_data["ym"]}')
            else:
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 360 查询剩余任务：{self.task_queue.qsize()} 过滤当前数据:{ym_data["ym"]}')
    ...
    def history_worker(self):
        ...
        while True:
            ...
            self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 历史 查询剩余任务：{self.task_queue.qsize()} 当前数据:{ym_data["ym"]}')
            history_info = history_obj.get_history(ym_data)
            if history_info == None:
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 历史 查询失败  域名有问题 {ym_data["ym"]} {ym_data["token"]}')
                continue

            if history_info == False:
                self.task_queue.put(ym_data)
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 历史 查询失败  重新查询 {ym_data["ym"]} {ym_data["token"]}')
                continue
            self.out_ym.insert_one({'ym': ym_data['ym'], 'type': 'main', 'filter_id':self.filter_id})

            try:
                if history_info['count'] == None:
                    self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 历史 查询剩余任务：{self.task_queue.qsize()}  过滤域名 {ym_data["ym"]}')
                    continue
                if int(history_info['count']) > 0 :
                    # 有直接放入redis 没有过滤
                    self.save_mysql(ym_data, 'history', history_info)
                    self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 历史 查询剩余任务：{self.task_queue.qsize()}  插入购买查询队列中 {ym_data["ym"]}')
                else:
                    # 有直接放入redis 没有过滤
                    self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 历史 查询剩余任务：{self.task_queue.qsize()}  过滤域名 {ym_data["ym"]}')
            except Exception as e:
                # 有直接放入redis 没有过滤
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 历史 查询剩余任务：{self.task_queue.qsize()}  过滤域名 {ym_data["ym"]} 错误:{e}')
    ...
    def wu_work(self):
        ...
        while True:
            if self.task_queue.empty():
                ...
                continue
            ym_data = self.task_queue.get()
            self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 查询剩余任务：{self.task_queue.qsize()} 当前数据:{ym_data["ym"]}')
            self.save_mysql(ym_data, 'wu', 'wu')
            # self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 插入购买查询队列中 {ym_data}')
    ...
    def zcs_worker(self):
        while True:
            ...
            if ym_data['zcs'] != '':
                ...
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 注册商 查询剩余任务：{self.task_queue.qsize()}  插入购买查询队列中 {ym_data["ym"]}')
            else:
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 注册商 查询剩余任务：{self.task_queue.qsize()} 过滤当前数据:{ym_data["ym"]}')
    ...
    def aizhan_worker(self):
        ...
        while True:
            ...
            if is_have == True:
                ...
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 爱站 查询剩余任务：{self.task_queue.qsize()}  插入购买查询队列中 {ym_data["ym"]}')
            else:
                self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 爱站 查询剩余任务：{self.task_queue.qsize()} 过滤当前数据:{ym_data["ym"]}')
    ...
    def index(self):
        ...
        self.p_id = os.getpid()
        self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 任务进程号：{self.p_id}')
        self.log_queue.put(f'{str(datetime.datetime.now())[:19]} 查询表单：{self.data}')
        # 修改状态 进行中
        self.update_spider_status('ym_yikoujia_jkt', self.filter['id'], 1)
